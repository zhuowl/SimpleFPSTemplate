...
void AFPSCharacter::Tick(float DeltaTime)
{
	...
	if (!IsLocallyControlled())
	{
		...
		NewRot.Pitch=RemoteViewPitch * 360.0f/ 255.0f;	//因为RemoteViewpitch是uint8格式的，需要转换回角度。
...
void AFPSCharacter::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	...
	//DOREPLIFETIME_CONDITION(AFPSCharacter, bIsCarryingObjective, COND_OwnerOnly);//属性仅发送给actor的所有者
}